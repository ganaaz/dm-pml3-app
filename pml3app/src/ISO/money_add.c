...
ISO8583_ERROR_CODES construct_transaction_response_object_for_money_add(MONEY_ADD_RESPONSE *resp_obj, ISO8583_TXN_RESP_OBJECT *resp_txn_obj)
{
...
ISO8583_ERROR_CODES process_money_add_transaction(MONEY_ADD_REQUEST *txn_obj, MONEY_ADD_RESPONSE *resp)
{
    ...
    memset(resp, 0x00, sizeof(MONEY_ADD_RESPONSE));

    ret = construct_transaction_request_object_for_money_add(req_txn_obj, txn_obj);
    if (ret != TXN_SUCCESS)
    {
        if (req_txn_obj)
        {
    ...
    ret = process_transaction(req_txn_obj, MONEY_ADD_HOST, &resp_txn_obj);
    ...
    ret = construct_transaction_response_object_for_money_add(resp, resp_txn_obj);
    if (ret != TXN_SUCCESS)
    {
        if (req_txn_obj)
            clear_transaction_object(req_txn_obj);
