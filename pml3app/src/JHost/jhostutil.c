...
int sendHostRequest(const char *body, const char *resourceName, char responseMessage[1024 * 32])
{
    ...
    if ((host = gethostbyname(appConfig.hostIP)) == NULL)
    {
        retStatus = -1;
        goto done;
    }
    ...
    addr.sin_port = htons(appConfig.hostPort);
    addr.sin_addr.s_addr = *(long *)(host->h_addr_list[0]);

    logData("Host names are set and going to connect the socket");
    ...
    logData("Socket Connect succeeded");
    ...
    logData("SSL Connect succeeded");
    char message[1024 * 16] = {0}; // Max message request is 16 kb
    generateMessage(body, resourceName, message);
...
void generateMessage(const char *body, const char *resourceName, char message[1024 * 32])
{
    ...
    sprintf(message + strlen(message), "%s%s%s", "Host: ", appConfig.httpsHostName, "\r\n");
