        ...
        for (int j = 0; j < 32; j++)
        {
            if (bitmap & 0x80000000 >> j)
            {
                ...
                case DE03_PROCESSING_CODE:
                    ret = copy_value(req_txn_obj, DE03_PROCESSING_CODE, txn_obj->DE03_PROC_CODE);
                    if (ret != TXN_SUCCESS)
                        return ret;
                    break;
                ...
                case DE13_TRANSACTION_DATE_LOCAL:
                    ...
                    break;
                    /*
                    case DE14_EXPIRATION_DATE:
                        ret = copy_value(req_txn_obj, DE14_EXPIRATION_DATE, txn_obj->DE14_EXPIRY_DATE);
                        if (ret != TXN_SUCCESS)
                            return ret;
                        break;
                        */
                ...
                case DE38_AUTHORIZATION_IDENTIFICATION_RESPONSE:
                    ret = copy_value(req_txn_obj, DE38_AUTHORIZATION_IDENTIFICATION_RESPONSE, txn_obj->DE38_AUTH_CODE);
                    if (ret != TXN_SUCCESS)
                        return ret;
                    break;

                case DE39_RESPONSE_CODE:
                    ret = copy_value(req_txn_obj, DE39_RESPONSE_CODE, txn_obj->DE39_RESPONSE_CODE);
                    if (ret != TXN_SUCCESS)
                        return ret;
                    break;
                ...
                case DE63_RESERVED_PRIVATE_3:
                    ret = copy_value(req_txn_obj, DE63_RESERVED_PRIVATE_3, txn_obj->DE63_PRIVATE_DATA.value);
                    if (ret != TXN_SUCCESS)
                        return ret;
                    break;
...
ISO8583_ERROR_CODES construct_transaction_response_object_for_reversal(REVERSAL_RESPONSE *resp_obj, ISO8583_TXN_RESP_OBJECT *resp_txn_obj)
{
...
ISO8583_ERROR_CODES process_reversal_transaction(REVERSAL_REQUEST *txn_obj, REVERSAL_RESPONSE *resp, int txn)
{
    ...
    memset(resp, 0x00, sizeof(REVERSAL_RESPONSE));

    ret = construct_transaction_request_object_for_reversal(req_txn_obj, txn_obj, txn);
    if (ret != TXN_SUCCESS)
    {
        if (req_txn_obj)
        {
    ...
    ret = process_transaction(req_txn_obj, txn, &resp_txn_obj);
    ...
    ret = construct_transaction_response_object_for_reversal(resp, resp_txn_obj);
    if (ret != TXN_SUCCESS)
    {
        if (req_txn_obj)
            clear_transaction_object(req_txn_obj);
